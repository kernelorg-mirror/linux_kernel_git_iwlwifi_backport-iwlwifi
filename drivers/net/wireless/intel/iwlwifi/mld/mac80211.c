				       ...
				       struct ieee80211_vif *vif)
{
	...
	debugfs_remove(iwl_mld_vif_from_mac80211(vif)->dbgfs_slink_mvm);
#endif
#endif
...
static void
iwl_mld_smps_workaround(struct iwl_mld *mld, struct ieee80211_vif *vif,
			bool enable_ps)
{
	struct iwl_mld_vif *mld_vif = iwl_mld_vif_from_mac80211(vif);
	 ...
	 * determine SMPS mode.
	 */
	if (mld_vif->ps_disabled == !enable_ps)
		return;

	mld_vif->ps_disabled = !enable_ps;
				       ...
				       enum ieee80211_sta_state new_state)
{
	...
	if (old_state == IEEE80211_STA_AUTHORIZED &&
		...
		if (!sta->tdls) {
			...
			/* vif->cfg.ps night still be false, but we no longer
			 * want to consider this vif when deciding the
			 * device-level power save
			 */
