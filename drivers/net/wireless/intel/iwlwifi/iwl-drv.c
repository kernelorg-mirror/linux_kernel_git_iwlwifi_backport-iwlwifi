...
{
	const struct iwl_mac_base *mac = trans->mac_cfg->base;
	const struct iwl_rf_cfg *cfg = trans->cfg;

	if (!mac->ucode_api_max) {
	...
	if (!cfg->ucode_api_max) {
		*api_min = mac->ucode_api_min;
		*api_max = mac->ucode_api_max;
		return;
	}

	*api_min = max(cfg->ucode_api_min, mac->ucode_api_min);
	*api_max = min(cfg->ucode_api_max, mac->ucode_api_max);
}
...
static int iwl_request_firmware(struct iwl_drv *drv, bool first)
{
	 ...
	 */
	if ((drv->fw_index != ucode_api_max
#ifdef CPTCFG_IWLWIFI_SUPPORT_DEBUG_OVERRIDES
	     && !drv->trans->dbg_cfg.load_old_fw
#endif
	    ) ||
	    drv->fw_index < ucode_api_min) {
#else
